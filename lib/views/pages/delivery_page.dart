...
class DeliveryUpdatePage extends StatefulWidget {
  const DeliveryUpdatePage({super.key, required this.deliveryID, required this.delivery});
...
class _DeliveryPageState extends State<DeliveryUpdatePage> {
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  ...
                  hapistoreDropdown(),
                  KForms.txtFormMoney('Order Amount', txtOrderAmount, (bool hasFocus, TextEditingController controller) => onFocusChange(hasFocus, controller), isEnabled: isDealer),
                  ...
                  // if updating an existing delivery record
                    ...
                    if (dropdownStatus.text == DeliveryStatus.delivered)...[
                      KForms.txtFormMoney('Cash Amount', txtCashAmount, (bool hasFocus, TextEditingController controller) => onFocusChange(hasFocus, controller), isRequired: false),
                      KForms.txtFormMoney('Online Amount', txtOnlineAmount, (bool hasFocus, TextEditingController controller) => onFocusChange(hasFocus, controller), isRequired: false),
                      KForms.txtFormMoney('Credit Amount', txtCreditAmount, (bool hasFocus, TextEditingController controller) => onFocusChange(hasFocus, controller), isRequired: false),
                      KForms.txtFormMoney('Return Amount', txtReturnAmount, (bool hasFocus, TextEditingController controller) => onFocusChange(hasFocus, controller), isRequired: false),
                    ...
                    if (dropdownStatus.text == DeliveryStatus.delivered || dropdownStatus.text == DeliveryStatus.returned)...[
                      KForms.txtAreaFormString('Remarks', txtRemarks, isRequired: (dropdownStatus.text == DeliveryStatus.returned || (dropdownStatus.text == DeliveryStatus.delivered && txtReturnAmount.text.isNotEmpty)))
                    ],
              
                    KForms.lastUpdatedByDetails(widget.delivery.createdBy, widget.delivery.createdDate,widget.delivery.lastUpdatedBy, widget.delivery.lastupdatedDate),

                    Column(
                      spacing: 5,
                      children: [
                        if (isDealer) KForms.regularButton('Delete', KButtonStyle.delete, () => KForms.alertDialogConfirm(ConfirmTitle.delete, ConfirmMessage.delete, context, onDelete)),
  ...
  void onSave() async {
    if (_formkey.currentState!.validate()){
      ...
      if (image != null) imageFilePath = await Helperfunctions.saveImage(context, image!);
  ...
  List<String> validate(){
    ...
    if (dropdownStatus.text == DeliveryStatus.delivered && totalAmount != orderAmount) listError.add('Total amount does not match the order amount!');
    if ((returnAmount != Decimal.zero || dropdownStatus.text == DeliveryStatus.returned) && txtRemarks.text.isEmpty) listError.add('Please enter a remark for return details!'); 
  ...
  void onDelete() async {
    showLoading(true);
    if (widget.delivery.imagePath.isNotEmpty) await Helperfunctions.deleteImage(context, widget.delivery.imagePath);
