            ...
            children: [
              ...
              Row(
                children: [
                  Expanded(
                    flex: 2,
                    child: Align(alignment: Alignment.centerLeft, child: KForms.textTitle('Amount')),
                  ),
                  Expanded(flex: 2, child: KForms.textTitle('Quanity')),
                  Expanded(
                    flex: 3,
                    child: Align(alignment: Alignment.centerRight, child: KForms.textTitle('Subtotal')),
                  ),
                ],
              ),
              ...
              breakdownRow(
                '₱1,000  x   ',
                txt1000,
                Helperfunctions.formatDoubleAmountForDisplay(txt1000.text.isEmpty ? 0 : double.parse(txt1000.text) * 1000),
              ),
              breakdownRow(
                '₱500  x   ',
                txt500,
                Helperfunctions.formatDoubleAmountForDisplay(txt500.text.isEmpty ? 0 : double.parse(txt500.text) * 500),
              ),
              breakdownRow(
                '₱200  x   ',
                txt200,
                Helperfunctions.formatDoubleAmountForDisplay(txt200.text.isEmpty ? 0 : double.parse(txt200.text) * 200),
              ),
              breakdownRow(
                '₱100  x   ',
                txt100,
                Helperfunctions.formatDoubleAmountForDisplay(txt100.text.isEmpty ? 0 : double.parse(txt100.text) * 100),
              ),
              breakdownRow('₱50  x   ', txt50, Helperfunctions.formatDoubleAmountForDisplay(txt50.text.isEmpty ? 0 : double.parse(txt50.text) * 50)),
              breakdownRow(
                '₱20 Bill x   ',
                txtB20,
                Helperfunctions.formatDoubleAmountForDisplay(txtB20.text.isEmpty ? 0 : double.parse(txtB20.text) * 20),
              ),
              breakdownRow(
                '₱20 Coin x   ',
                txtC20,
                Helperfunctions.formatDoubleAmountForDisplay(txtC20.text.isEmpty ? 0 : double.parse(txtC20.text) * 20),
              ),
              ...
              breakdownRow(
                'Centavo   ',
                txtCent,
                Helperfunctions.formatDoubleAmountForDisplay(txtCent.text.isEmpty ? 0 : double.parse(txtCent.text) * .01),
              ),
              ...
              KForms.lefRightLabel(
                'Discrepancy : ',
                Helperfunctions.formatDoubleAmountForDisplay(widget.breakdown.discrepancy),
                rightLabelColor: Colors.red,
              ),
              KForms.switchYesNo('With Bank Deposit?', withBankDeposit, () {
              ...
              if (widget.breakdownID.isNotEmpty) ...[
                KForms.lastUpdatedByDetails(
                  widget.breakdown.createdBy,
                  widget.breakdown.createdDate,
                  widget.breakdown.lastUpdatedBy,
                  widget.breakdown.lastupdatedDate,
                ),
  ...
  Widget breakdownRow(String denomination, TextEditingController controller, String subtotal) {
    return Row(
      children: [
        Expanded(
          flex: 2,
          child: Align(alignment: Alignment.centerRight, child: KForms.textDescriptionString(denomination)),
        ),
        Expanded(flex: 2, child: KForms.txtFormNumber('', controller, isRequired: false, onEditingComplete: recompute)),
        Expanded(
          flex: 3,
          child: Align(alignment: Alignment.centerRight, child: KForms.textDescriptionString(subtotal)),
        ),
      ],
    );
  }
