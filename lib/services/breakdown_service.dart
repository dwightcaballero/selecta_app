...
class BreakdownService {
  ...
  BreakdownService(){
    _breakdownRef = _firestore.collection(BREAKDOWN_COLLECTION_REF).withConverter<Breakdown>(
      fromFirestore: (snapshots, _) => Breakdown.fromJson(snapshots.data()!), 
      toFirestore: (breakdown, _) => breakdown.toJson());
  }

  Future<Breakdown?> getDocumentsBySpecificDate(DateTime breakdownDate) async {
    DateTime startOfDay = DateTime(breakdownDate.year, breakdownDate.month, breakdownDate.day);
    DateTime endOfDay = DateTime(breakdownDate.year, breakdownDate.month, breakdownDate.day, 23, 59, 59);

    try {
      QuerySnapshot querySnapshot = await FirebaseFirestore.instance
          .collection(BREAKDOWN_COLLECTION_REF)
          .where('breakdownDate', isGreaterThanOrEqualTo: Timestamp.fromDate(startOfDay))
          .where('breakdownDate', isLessThanOrEqualTo: Timestamp.fromDate(endOfDay))
          .limit(1)
          .get();

      if (querySnapshot.docs.isEmpty) return null;

      return querySnapshot.docs.map((doc) {return Breakdown.fromJson(doc.data() as Map<String, dynamic>);}).first;
