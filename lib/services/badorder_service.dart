...
class BadOrderService {
  ...
  BadOrderService(){
    _badorderRef = _firestore.collection(BADORDER_COLLECTION_REF).withConverter<BadOrder>(
  ...
  Future<double?> getTotalBadOrderForSpecificDay(DateTime badorderDate) async {
    ...
    try {
      ...
      Query query = FirebaseFirestore.instance
          ...
          .where('badorderDate', isGreaterThanOrEqualTo: Timestamp.fromDate(startOfDay))
          .where('badorderDate', isLessThanOrEqualTo: Timestamp.fromDate(endOfDay));

      // 2. Attach the sum aggregate function
      AggregateQuerySnapshot snapshot = await query.aggregate(sum('badorderAmount')).get();
      ...
      print("Error calculating total bad order amount for end of day: $e");
      return null;
    }
  }
