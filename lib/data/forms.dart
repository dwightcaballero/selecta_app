...
class KForms{
  ...
  static Focus txtFormMoney(String label, TextEditingController controller, Function(bool hasFocus, TextEditingController controller) onFocusChange, {bool isRequired = true, isEnabled = true}){
    return Focus(
    ...
    return InputDecorator(
      ...
      child: Row(
        children: [
          ...
          Spacer(),
          FilledButton(
            onPressed: isEnabled? onChangeDate : null,
            child: Text("Choose"))
        ],
      ),
    );
  ...
  static DropdownMenuFormField<String> dropdown(String label, List<DropdownMenuEntry<String>> dropdownItems, TextEditingController dropDownController, {VoidCallback? onSelected, isenabled = true}){
  ...
  static Row switchYesNo(String title, bool isToggled, VoidCallback onToggle){
    return Row(
      children: [
        KForms.textTitle(title), Spacer(),
        ToggleSwitch(
          initialLabelIndex: isToggled ? 1 : 0, // Map boolean to index
          totalSwitches: 2,
          labels: ['No', 'Yes'],
          activeBgColors: [[Colors.red[300]!], [Colors.green[300]!]],
          onToggle: (index) {
            onToggle();
          },
        ),
  ...
  static Row lefRightLabel(String leftLabel, String rightLabel, {Color rightLabelColor = Colors.black, bool withLeftIndent = false}){
    return Row(
      children: [
        ...
        KForms.textTitle(leftLabel), Spacer(),
  ...
  static Column lastUpdatedByDetails(String createdBy, Timestamp createdDate, String lastUpdatedBy, Timestamp lastUpdatedDate,){
    return Column(
      ...
      children: [
        Divider(),
        Text('Created By: $createdBy\nCreated Date: ${DateFormat('E, d MMM yyyy, hh:mm a').format(createdDate.toDate())}\n\nUpdated By: $lastUpdatedBy\nUpdated Date: ${DateFormat('E, d MMM yyyy, hh:mm a').format(lastUpdatedDate.toDate())}', style: KTextStyle.descriptionTextStyle,),
        Divider(),
      ],
    );
  }
  ...
  static Row documentScanner(String title, BuildContext context, File? image, String networkImagePath, Function (File? scannedImage) scanDocs, {bool isEnabled = true}){
    bool hasImageData = (image != null || networkImagePath.isNotEmpty);
        ...
        Expanded(
          child: Column(
            ...
            children: [
              ...
              FilledButton.icon(
                onPressed: !isEnabled 
                  ...
                  try {
                    ...
                    scannedData = null;
                    if (context.mounted) ShowMessage.error(context, e.message ?? 'There was an error upon scanning a document');
                  }
              ...
              FilledButton.icon(
                onPressed: image == null && networkImagePath.isEmpty && isEnabled ? null : () {
                  image = null;
                  networkImagePath = '';
                  scanDocs(null);
                },
